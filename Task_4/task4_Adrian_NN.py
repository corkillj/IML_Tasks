...
train_X = np.zeros((train_txt.shape[0],1536))
train_y = np.zeros((train_txt.shape[0],1))

for i in range(0,train_X.shape[0]):
    train_X[i,0:512] = features[train_txt[i,0],:]
    if(random.random()>0.5):
       train_X[i,512:1024] = features[train_txt[i,1],:]
       train_X[i,1024:1536] = features[train_txt[i,2],:]
       train_y[i] = 1
    else:
       train_X[i,512:1024] = features[train_txt[i,2],:]
       train_X[i,1024:1536] = features[train_txt[i,1],:]
       train_y[i] = 0
...
X_te = np.concatenate((X_te,np.concatenate((X_te[:,0:512],X_te[:,1024:1536],X_te[:,512:1024]),axis=1)),axis = 0)
X_tr = np.concatenate((X_tr,np.concatenate((X_tr[:,0:512],X_tr[:,1024:1536],X_tr[:,512:1024]),axis=1)),axis = 0)
...
class binaryClassification(nn.Module):
    def __init__(self):
        ...
        # Number of input features is 1536.
        self.layer_1 = nn.Linear(1536, 2048) 
        self.layer_2 = nn.Linear(2048, 1024)
        self.layer_out = nn.Linear(1024, 1) 
...
test_X = np.zeros((test_txt.shape[0],1536))

for i in range(0,test_X.shape[0]):
    test_X[i,0:512] = features[test_txt[i,0],:]
    test_X[i,512:1024] = features[test_txt[i,1],:]
    test_X[i,1024:1536] = features[test_txt[i,2],:]
...
# %%
np.savetxt('task4_adi_nn.txt',y_out,fmt="%i")
